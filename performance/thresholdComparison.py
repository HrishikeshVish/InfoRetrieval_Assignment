...
def threshold():
    ...
    for ele in queries:
        ...
        if(len(ranked_order_of_docs)>2500):
            ranked_docs = rankByTFIDF(ranked_order_of_docs[0:3500], 5, ele)
        else:
        ...
        for ele in elasticFiles:
            ...
            if ele in similarity_scores.keys():
                if(similarity_scores[ele] > 0.8):
                    actualValues.append(ele)
        ...
        while(j<remVals):
            if(actULs[tempcount] not in actualValues):
                ...
                tempcount+=1
        with open('./performance/actual/'+str(counter)+'.json', "w") as f:
